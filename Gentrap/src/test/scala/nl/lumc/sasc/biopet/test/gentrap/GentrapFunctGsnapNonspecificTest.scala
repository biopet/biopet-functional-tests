...
trait GentrapFunctional extends Gentrap with Rna1 with Rna2 {
  ...
  def testFragmentsPerGene(): Unit = {
    if (this.isInstanceOf[FragmentsPerGene]) {
      val fixtureFile =
        Biopet.fixtureFile("gentrap", "count_table", "func1", "all_samples.fragments_per_gene")
      val pipelineFile = new File(
        outputDir,
        "expression_measures/fragmentspergene/fragmentspergene.fragments_per_gene.tsv")
  ...
  def baseCountsProvider = {
    Array(
      "basecounts.exonAntiSenseCounts.tsv",
      ...
      x =>
        Array(Biopet.fixtureFile("gentrap", "count_table", "func1", "basecounts", x),
  ...
  def testCufflinksStrictGenes(): Unit = {
    if (this.isInstanceOf[CufflinksStrict]) {
      val fixtureFile = Biopet.fixtureFile("gentrap",
                                           "count_table",
                                           "func1",
                                           "all_samples.genes_fpkm_cufflinks_strict")
      val pipelineFile =
        new File(outputDir, "expression_measures/cufflinksstrict/cufflinksstrict.genes.fpkm.tsv")
      pearsonScoreTest(fixtureFile, pipelineFile)
    }
  }
  ...
  def testCufflinksStrictIsoform(): Unit = {
    if (this.isInstanceOf[CufflinksStrict]) {
      val fixtureFile = Biopet.fixtureFile("gentrap",
                                           "count_table",
                                           "func1",
                                           "all_samples.isoforms_fpkm_cufflinks_strict")
      val pipelineFile = new File(
        outputDir,
        "expression_measures/cufflinksstrict/cufflinksstrict.iso_form.fpkm.tsv")
      pearsonScoreTest(fixtureFile, pipelineFile)
    }
  }
...
object GentrapFunctional {
  ...
  def pearsonScoreTest(tableA: File, tableB: File, minScore: Double, samples: Set[String]): Unit = {
    samples.foreach { sample =>
      val pValue = pearsonScore(loadMergedCountTable(tableA, sample),
                                loadMergedCountTable(tableB, sample)).getOrElse(0.0)
...
class GentrapFunctGsnapNonspecificTest
    extends GentrapFunctional
    with GentrapSuccess
    with Gsnap
  ...
  def annotationRefflat =
    Option(Biopet.fixtureFile("gentrap", "annotations", "ucsc_refseq.refFlat"))
