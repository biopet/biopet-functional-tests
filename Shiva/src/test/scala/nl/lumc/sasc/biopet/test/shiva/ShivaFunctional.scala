...
import nl.lumc.sasc.biopet.test.references.{ HsapiensGRCh37_p13_no_alt_analysis_set, HsapiensGRCh38_no_alt_analysis_set, HsapiensHg19 }
 ...
 */
class ShivaBiopetplanet30xGRCh37Test extends ShivaSuccess with BwaMem with HsapiensGRCh37_p13_no_alt_analysis_set with NA12878Bioplanet30x
...
class ShivaBiopetplanet30xGRCh38Test extends ShivaSuccess with BwaMem with HsapiensGRCh38_no_alt_analysis_set with NA12878Bioplanet30x
  with Haplotypecaller with HaplotypecallerGvcf with Unifiedgenotyper {
  override def disablescatter = false
  def paired = true
  def shouldHaveKmerContent = Some(false)
