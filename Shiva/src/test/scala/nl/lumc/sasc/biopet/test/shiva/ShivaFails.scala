...
class ShivaNoOutputDirTest extends Shiva with PipelineFail with BwaMem with TestReference with Haplotypecaller with Wgs1 {
  override def outputDirArg = None
  logMustNotHave("""FunctionEdge - Starting""".r)
  logMustHave("""Value does not exist but is required, key: output_dir""".r)
}

class ShivaWrongBamHeaderTest extends Shiva with PipelineFail with BwaMem with TestReference
  with Haplotypecaller with Wgs1WrongBam {
  logMustNotHave("""FunctionEdge - Starting""".r)
  logMustHave("""Sample readgroup and/or library of input bamfile is not correct,""".r)
}

class ShivaWrongMd5Test extends Shiva with PipelineFail with BwaMem with TestReference with Haplotypecaller with Wgs1 {
