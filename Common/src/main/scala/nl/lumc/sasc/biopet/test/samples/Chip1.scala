  ...
  val lib1ConfigMap = Map("samples" ->
    Map(s"$sampleName" ->
      Map("libraries" ->
        Map("lib1" ->
          Map(
            "R1" -> Biopet.fixtureFile("samples", sampleName, "sample1_R1.fastq.gz").getAbsolutePath,
            "R1_md5" -> "d538c6e674b7152cf201a10dd75f08e8",
            "R2" -> Biopet.fixtureFile("samples", sampleName, "sample1_R2.fastq.gz").getAbsolutePath,
